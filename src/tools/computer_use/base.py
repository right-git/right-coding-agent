...
class ComputerUse:
    ...
    def require_window(self, title_contains: str):
        ...
        if window is None or title_contains.casefold() not in window.title.casefold():
            raise RuntimeError(
                f"expected {title_contains!r} to be focused, but it is "
    ...
    def _refine(
        ...
        if not found:
            ...
            return detection
        return Detection(
            label=found[0].label, box=shift_box(found[0].box, (area[0], area[1]))
        )
    ...
    def locate_object(
        ...
        *,
        ...
        if refine:
            detections = [
                self._refine(image, description, detection, scale)
                if index < self.refine_limit
                else detection
    ...
    def find_object(
        ...
        *,
        ...
        detections = self.locate_object(
            description, screenshot=screenshot, mode="first", region=region
        )
    ...
    def center_of(self, detection: Detection) -> Point:
        image_size = (
            self.last_screenshot.size
            if self.last_screenshot is not None
            else self.screen_size()
        )
    ...
    def describe(self, detections: Sequence[Detection] | None = None) -> str:
        ...
        return describe_detections(
            self.last_detections if detections is None else detections, image_size
        )
    ...
    def drag(
        self, start: Point, end: Point, *, button: MouseButton = "left"
    ) -> None:
