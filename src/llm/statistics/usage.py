...
The per-turn record itself (`TurnUsage`) lives in `src.llm.types`; the small
extractors and formatters live in `src.llm.utils`.
"""
...
from ..types import TurnUsage
from ..utils import embedded_tool_calls
...
def turn_usage_from_messages(
    ...
    for message in messages:
        ...
        if isinstance(message, ToolMessage):
            script_tool_calls += embedded_tool_calls(message.content)
            continue
