...
class TkOverlay:
    ...
    def _start(self) -> bool:
        ...
        with self._lock:
            if self._thread is not None and self._thread.is_alive():
                return True
            self._thread = threading.Thread(target=self._serve, name="computer-use-overlay", daemon=True)
            self._thread.start()
    ...
    def _serve(self) -> None:
        ...
        try:
            self._run_loop(root, tkinter, tkinter_font)
        finally:
            try:
                root.destroy()
            except Exception:
    ...
    def _run_loop(self, root, tkinter, tkinter_font) -> None:
        """Configure the window and pump commands until told to stop.

        Runs on the overlay thread. The handlers close over `root` and
        `canvas`; both live in this frame, which is gone by the time
        `_serve` tears the window down.
        """
        root.withdraw()
    ...
    def _draw(self, canvas, marker: Marker, screen_size: Size, tkinter_font) -> None:
        ...
        canvas.create_rectangle(x1, y1, x2, y2, outline=style.accent, width=style.outline_width)

        anchor = marker.anchor or box_center(marker.box, screen_size)
        title_font = tkinter_font.Font(family=style.font_family, size=style.title_size, weight="bold")
        ...
        content_width = max([title_font.measure(marker.title)] + [note_font.measure(line) for line in note_lines])
        content_height = title_height + (style.line_gap + note_height * len(note_lines) if note_lines else 0)
        tooltip_size = (
