...
EXTENDED_KEY_CODES = frozenset({0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x2C, 0x2D, 0x2E, 0x5B, 0x5C, 0x90})
...
def key_combination_events(combination: str) -> list[KeyEvent]:
    ...
    events = [KeyEvent(code=code, pressed=True, extended=code in EXTENDED_KEY_CODES) for code in codes]
    events.extend(KeyEvent(code=code, pressed=False, extended=code in EXTENDED_KEY_CODES) for code in reversed(codes))
    return events
...
class Win32InputDispatcher:
    ...
    def key(self, code: int, *, pressed: bool, extended: bool = False) -> None:
        ...
        entry.payload.ki = _KEYBDINPUT(wVk=code, wScan=0, dwFlags=flags, time=0, dwExtraInfo=0)
        self._send(entry)
    ...
    def unicode(self, code_unit: int, *, pressed: bool) -> None:
        ...
        if not pressed:
            flags |= KEYEVENTF_KEYUP
        entry = _INPUT(type=INPUT_KEYBOARD)
        entry.payload.ki = _KEYBDINPUT(wVk=0, wScan=code_unit, dwFlags=flags, time=0, dwExtraInfo=0)
        self._send(entry)
...
class Pointer:
    ...
    def _press_modifiers(self, modifiers: Sequence[str]) -> list[int]:
        ...
        for code in codes:
            self.dispatcher.key(code, pressed=True, extended=code in EXTENDED_KEY_CODES)
        return codes

    def _release_modifiers(self, codes: Iterable[int]) -> None:
        for code in reversed(list(codes)):
            self.dispatcher.key(code, pressed=False, extended=code in EXTENDED_KEY_CODES)
    ...
    def key(self, combination: str) -> None:
        for event in key_events(combination):
            self.dispatcher.key(event.code, pressed=event.pressed, extended=event.extended)
    ...
    def hold_key(self, combination: str, duration: float) -> None:
        ...
        try:
            ...
            for event in (event for event in events if not event.pressed):
                self.dispatcher.key(event.code, pressed=False, extended=event.extended)
