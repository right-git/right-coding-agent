...
from .utils import as_content_text, extract_front_matter
...
class WebParser:
    ...
    async def make_request(
        ...
        raise (last_exception if last_exception else RuntimeError("make_request failed unexpectedly"))

    def parse_html(self, html: str, metadata_primary: bool = False) -> dict[str, Any] | str:
        soup = BeautifulSoup(html, "html.parser")
        has_description_meta = any(
            str(meta.get("name") or "").lower() == "description" for meta in soup.find_all("meta")
        )
    ...
    async def parse_page(self, url: str) -> str:
        ...
        try:
            ...
            logger.warning(
                "Markdown conversion failed for [{}]: {}; " "falling back to plain text",
                url,
                error,
            )
            result = BeautifulSoup(html, "html.parser").get_text(" ", strip=True)
        return as_content_text(result)
