...
def scrub_text(text: str, max_chars: int = MAX_TEXT_CHARS) -> str:
    ...
    text = _DATA_URI.sub(lambda match: f"<data-uri stripped, {len(match.group(0))} chars>", text)
    text = _BASE64_RUN.sub(lambda match: f"<base64 stripped, {len(match.group(0))} chars>", text)
    ...
    def before_model(self, state, runtime=None) -> None:
        try:
            ...
            payload = {
                ...
                "messages": [serialize_message(message, self.max_text_chars) for message in messages],
            }
