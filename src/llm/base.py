...
from .types import LLMProvider, T, AgentTool, MiddlewareFactory
...
class LLMClient:
    ...
    def resolve_model_name(
        ...
        if not resolved_model_name:
            raise ValueError(
                f"Model name is not configured for provider {provider.provider_name}"
            )
            ...
            for attempt in range(self.num_retries):
                ...
                try:
                    ...
                    response = await agent.ainvoke(
                        input=agent_input, context=context, config=config
                    )
