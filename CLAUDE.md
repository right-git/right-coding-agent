...
- `MessageLogMiddleware` (`src/llm/log_middleware.py`) — logs model traffic to `logs.log` as self-describing JSON lines: `{"event": "model_request", ...}` with the full message list before every call, and `{"event": "model_response", ...}` with the returned AIMessage **including its `response_metadata` (finish_reason)** after it — that's how empty/aborted completions get diagnosed. Registered **last** so it sees the message list exactly as the model does. Its `scrub_text` replaces data URIs and long base64 runs with `<... stripped, N chars>` placeholders and truncates remaining long text; the terminal UI reuses it for tool-result previews. Tests inject `emit=` to capture lines.
